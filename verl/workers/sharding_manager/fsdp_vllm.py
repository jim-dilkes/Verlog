    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        if model.config.architectures[0] in ['DeepseekV2ForCausalLM', 'DeepseekV3ForCausalLM']:
            loaded_params = patched_ds_v3_load_weights(
                model, ((name, param.full_tensor() if world_size != 1 and hasattr(param, 'full_tensor') else param.to(device, non_blocking=True) if hasattr(param, 'to') else param)
                        for name, param in updated_params.items()))
        else:
            loaded_params = model.load_weights(
                ((name, param.full_tensor() if world_size != 1 and hasattr(param, 'full_tensor') else param.to(device, non_blocking=True) if hasattr(param, 'to') else param) 
                 for name, param in updated_params.items()))
